...
JSON_OUT_PATH = SVM_DIR / "svm_control_vs_adv_cv_raw_results_llama1b.json"
TXT_OUT_PATH = SVM_DIR / "svm_control_vs_adv_cv_raw_summary_llama1b.txt"

BEST_W_PATH = SVM_DIR / "w_control_adv_llama1b.npy"
BEST_B_PATH = SVM_DIR / "b_control_adv_llama1b.npy"
LABELS_BINARY_PATH = SVM_DIR / "binary_labels_control_vs_adv_raw_llama1b.npy"
INDICES_MAP_PATH = SVM_DIR / "indices_mapping_control_vs_adv_raw_llama1b.json"
...
def load_embeddings(path: Path, label: str) -> np.ndarray:
    ...
    if x.ndim != 2:
        raise ValueError(f"Expected 2D {label} embedding matrix at {path}, got shape {x.shape}")
    return x
...
def main() -> None:
    print(f"Loading adversarial embeddings from {ADV_EMBEDDINGS_PATH}...")
    x_adv = load_embeddings(ADV_EMBEDDINGS_PATH, "adversarial")
    print(f"Loaded adversarial embeddings shape: {x_adv.shape}")

    print(f"Loading control embeddings from {CONTROL_EMBEDDINGS_PATH}...")
    x_control = load_embeddings(CONTROL_EMBEDDINGS_PATH, "control")
    print(f"Loaded control embeddings shape: {x_control.shape}")

    if x_adv.shape[1] != x_control.shape[1]:
        raise ValueError(
            f"Feature mismatch: adv has {x_adv.shape[1]} dims, control has {x_control.shape[1]} dims"
        )
    ...
    INDICES_MAP_PATH.write_text(
        json.dumps(
            {
                "label_definition": {"0": "control", "1": "adversarial"},
                "control_indices_in_joint_matrix": list(range(len(x_control))),
                "adversarial_indices_in_joint_matrix": list(range(len(x_control), len(x_control) + len(x_adv))),
                "control_original_indices": list(range(len(x_control))),
                "adversarial_original_indices": list(range(len(x_adv))),
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    print("\nTraining subset (Control vs Adversarial):")
    print(f"  Control (0): {(y == 0).sum()}")
    print(f"  Adversarial (1): {(y == 1).sum()}")
    print(f"  Total: {len(y)}")
    ...
    class_counts = {
        "0_control": int((y == 0).sum()),
        "1_adversarial": int((y == 1).sum()),
    }
    class_proportions = {
        "0_control": float((y == 0).mean()),
        "1_adversarial": float((y == 1).mean()),
    }
    ...
    results = {
        "model_name": "llama1b",
        "control_embeddings_path": str(CONTROL_EMBEDDINGS_PATH),
        "adversarial_embeddings_path": str(ADV_EMBEDDINGS_PATH),
        "subset_definition": "Binary classification on raw embeddings: control prompts vs adversarial prompts.",
        "label_definition": {"0": "control", "1": "adversarial"},
        "preprocessing": {
            "description": "none; raw embeddings used as stored",
            "note": "The saved w_control_adv_llama1b and b_control_adv_llama1b operate directly on the raw embeddings.",
        },
    ...
    lines = [
        "Linear SVM on llama1b prompt embeddings",
        "Binary Classification: Control vs Adversarial",
        f"Control embeddings: {CONTROL_EMBEDDINGS_PATH}",
        f"Adversarial embeddings: {ADV_EMBEDDINGS_PATH}",
        "",
        "Raw-embedding setup",
        ...
        "- Refits a final hyperplane on all samples with selected C and saves llama1b-named w/b files.",
        "",
        f"Samples: {x.shape[0]}",
        f"Features: {x.shape[1]}",
        "",
        "Label distribution",
        f"0 (control): {class_counts['0_control']} / {x.shape[0]} = {class_proportions['0_control']:.4f}",
        f"1 (adversarial): {class_counts['1_adversarial']} / {x.shape[0]} = {class_proportions['1_adversarial']:.4f}",
        "",
        ...
        f"Saved w_control_adv_llama1b: {BEST_W_PATH}",
        f"Saved b_control_adv_llama1b: {BEST_B_PATH}",
        "",
        "Interpretation note",
        "The saved w_control_adv_llama1b and b_control_adv_llama1b apply directly to raw embeddings.",
    ]
    ...
    print(f"Saved final hyperplane w_control_adv_llama1b to {BEST_W_PATH}")
    print(f"Saved final hyperplane b_control_adv_llama1b to {BEST_B_PATH}")
